...
import { Camera, CheckCircle, XCircle, Loader2, X, QrCode, Keyboard } from 'lucide-react';
...
export const QRScanner: React.FC<QRScannerProps> = ({ onClose, onSuccess }) => {
  ...
  const handleQRData = async (rawData: string) => {
    try {
      ...
      if (!qrData.admin_session_id || !qrData.session_token) {
  ...
  const verifyQR = async (qrData: { admin_session_id: string; session_token: string }) => {
    setStatus('verifying');

    try {
      const { data, error } = await supabase.functions.invoke('verify-admin-qr', {
        body: qrData,
      });
      ...
      setStatus('success');
      toast.success('PC login authorized!');
        ...
        return (
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-sm text-muted-foreground text-center">
              Point your camera at the QR code on the PC screen
            </p>
      ...
      case 'manual':
        return (
          <div className="flex flex-col gap-4">
            ...
            <p className="text-sm text-muted-foreground text-center">
              Copy the QR code data from the PC screen and paste it below
            </p>
      ...
      case 'success':
        return (
          <div className="flex flex-col items-center justify-center py-12 gap-4">
            ...
            <h3 className="text-xl font-semibold text-success">PC Login Authorized!</h3>
            <p className="text-sm text-muted-foreground text-center">
              You can now use the app on your PC
            </p>
          </div>
        );
    ...
    <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="relative">
          ...
          <CardTitle className="text-center">Scan QR to Login PC</CardTitle>
          <CardDescription className="text-center">
            Authorize your PC session by scanning the QR code
          </CardDescription>
        </CardHeader>
        <CardContent>
          {renderContent()}
